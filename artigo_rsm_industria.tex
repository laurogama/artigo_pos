...
The central hub was implemented also using raspberry pi 2 model B running a computer program called \textit{"MQTT server"}, a RabbitMQ broker and  a microwebserver based on the framework flask\cite{Flask}.

The server was responsible to listen for messages posted on the RabbitMQ broker and collect the relevant data and persist them on a SQLite\cite{Sqlite2015} database.

The RabbitMQ broker was responsible for managing communications between nodes and the central hub.

The microwebserver was an application with an REST API\cite{Fielding2000} responsible to provide other devices with an method to visualize the data collected by the sensor network.
