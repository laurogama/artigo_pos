...
\and
\IEEEauthorblockN{Antonio de Padua Soares Junior}
\IEEEauthorblockA{Universidade do Estado do Amazonas\\
Manaus, Amazonas\\
...
The analysis of this data may help prevent violations in work conditions and enviromental damage. Low worker productivity and mood disorders are also often related to to bad work enviroments and be combated by analysis of the enviroment.
...
The sensor nodes are raspberry pi 2 model B \cite{Raspberry2015} running a Raspbian\cite{Raspbian2015} distribution composed of a microcontroller receiving signals from sensors.
...
The sensor node has a program called \textit{"client"} that is responsible for the data collection and transmission.

The client persists data from the sensors in a SQLite\cite{Sqlite2015} database and transmits this information to the central hub.
...
The central hub was implemented also using raspberry pi 2 model B running a computer program based on Tornado\cite{Tornado2015}, a RabbitMQ broker and  a webserver based on the framework flask\cite{Flask}.
...
The webserver is an application with an REST API\cite{Fielding2000} responsible for providing users of the system with an method to visualize the data collected by the sensor network. It uses a lightweight server structure ideal for running in embedded systems.
...
The model takes into account the need for constant data analysis and transmission of high volumes of messages in a heavy electronic noise environment.
The presented architecture is able to be scaled to a high number of nodes.
